...
class TableMultiAPI {
  ...
  // GraphQL
  async gql_create( props, return_fields = [] ){
    ...
    // GraphQL requires that we specify the fields we want it to return
  ...
  // @param Object primary_key
  // @param Array return_fields
  async cql_read( primary_key, return_fields = [] ){
  ...
  // defined in props
  async rest_update( props, primary_key ){
    return await post(true, 'PUT')( `${restURL}/v1/keyspaces/${keyspace}/tables/${this.tablename}/rows/${primary_key.join(';')}`,
                          { columns: props_to_columns( props ) })
  }
  ...
  // @param Object primary_key
  // @param Array return_fields
  async gql_delete( primary_key, return_fields = [] ){
    if (return_fields == []) throw new Error('GraphQL API requires specifying return fields')
    let prop_string = make_special_json( camelcase_props( primary_key ))
    let return_fields_string = return_fields.map(camelcase).join(' ')
    let GqlModel = camelcase( this.tablename, {pascalCase: true} )
